...
class Chord(ChordBase):
    ...
    **Equality**
    ...
    @cacheMethod
    def isIncompleteSeventh(self) -> bool:
        '''
        Gibt ``True`` zurück, wenn der Akkord ein unvollständiger Septakkord ist.
        Dafür muss er genau drei unterschiedliche Tonnamen enthalten. Grundton
        und Septime müssen vorhanden sein; von Terz und Quinte muss genau einer
        der beiden Akkordtöne vorhanden sein.

        >>> chord.Chord(['C', 'G', 'B-']).isIncompleteSeventh()
        True
        >>> chord.Chord(['C', 'E', 'B-']).isIncompleteSeventh()
        True
        >>> chord.Chord(['C', 'E', 'G']).isIncompleteSeventh()
        False
        >>> chord.Chord(['C', 'E', 'G', 'B-']).isIncompleteSeventh()
        False
        '''
        uniquePitchNames = set(self.pitchNames)
